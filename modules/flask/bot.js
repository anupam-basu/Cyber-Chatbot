...
function botdelay() {
  module.exports = botBuilder(function (request) {
    ...
    if (hash == "")
      // if a hash is not mentioned in the query, we get the hash from the file
      return getHashFromFile(query);
    else return replytoquestion(query);
  });
}
...
function getHashFromFile(query) {
  ...
  return new Promise((resolve, reject) => {
    ...
    .then((data) => replytoquestion(query))
  ...
  return fetch(
    ...
    .then(function (data) {
      ...
      if (flag == 0) {
        ...
        if (notunknown == 0) strfam = strfam + " Unknown";
        if (query == qvirus) {
          if (mal > 0) {
            str = str + replyvirus + String(mal) + " model(s)";
          } else str = str + replynotvirus;
        }
        if (query == qfamily) {
      ...
      var jsontext =
        '{ "text": "' + str + '" , "response_type": "in_channel"  }';
